...
class TestIngrediente(TestCase):
    ...
    def test_crear_ingrediente(self):
        ...
        endpoint_ingredientes = "/ingredientes"
        headers = {
            "Content-Type": "application/json",
            "Authorization": "Bearer {}".format(self.token),
        }

        resultado_nuevo_ingrediente = self.client.post(
            endpoint_ingredientes, data=json.dumps(nuevo_ingrediente), headers=headers
        )
        ...
        self.assertEqual(
            float(datos_respuesta["calorias"]), float(ingrediente.calorias)
        )
    ...
    def test_editar_ingrediente(self):
        ...
        # Definir endpoint, encabezados y hacer el llamado
        endpoint_ingredientes = "/ingrediente/" + str(ingrediente.id)
        headers = {
            "Content-Type": "application/json",
            "Authorization": "Bearer {}".format(self.token),
        }

        resultado_edicion_ingrediente = self.client.put(
            endpoint_ingredientes, data=json.dumps(ingrediente_editado), headers=headers
        )
        ...
        self.assertNotEqual(
            float(datos_respuesta["calorias"]), calorias_nuevo_ingrediente
        )
        ...
        self.assertEqual(
            float(datos_respuesta["calorias"]), calorias_ingrediente_editado
        )
    ...
    def test_borrar_ingrediente(self):
        ...
        resultado_borrado_ingrediente = self.client.delete(
            endpoint_ingredientes, headers=headers
        )
    ...
    def test_dar_ingrediente(self):
        ...
        resultado_consulta_ingrediente = self.client.get(
            endpoint_ingredientes, headers=headers
        )
        ...
        self.assertEqual(
            float(datos_respuesta["costo"]), float(costo_nuevo_ingrediente)
        )
        self.assertEqual(
            float(datos_respuesta["calorias"]), float(calorias_nuevo_ingrediente)
        )
        ...
        for ingrediente in datos_respuesta:
            for ingrediente_creado in self.ingredientes_creados:
                if ingrediente["id"] == str(ingrediente_creado.id):
                    ...
                    self.assertEqual(
                        float(ingrediente["costo"]), float(ingrediente_creado.costo)
                    )
                    self.assertEqual(
                        float(ingrediente["calorias"]),
                        float(ingrediente_creado.calorias),
                    )
